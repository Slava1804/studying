...
def play(word):
    word_completion = ['_' for _ in range(len(word))]  # список, содержащий символы _ на каждую букву задуманного слова
    guessed_letters = []                               # список уже названных букв
    guessed_words = []                                 # список уже названных слов
    tries = 6                                          # количество попыток
    res_word = [word[i] for i in range(len(word))]
    print('Давайте играть в угадайку слов!')
    print(display_hangman(tries))
    print(* word_completion, '- Загаданное слово')
    while True:
        sym = input('Введите букву или слово: ').upper()
        if sym == word:
            ...
            if not sym.isalpha():
                ...
                continue
            elif sym not in guessed_letters or guessed_words:
                if len(sym) > 1:
                    guessed_words.append(sym)
                else:
                    guessed_letters.append(sym)
            elif sym in guessed_letters or sym in guessed_words:
                tries -= 1
                print(display_hangman(tries))
                print('Уже было введено!')
                if tries == 0:
                    print(display_hangman(tries))
                    print(word, '- Загаданное слово.')
                    return 'Вы проиграли.'
        if sym in res_word:
            print('Такая', sym, 'буква есть!')
            t = word.find(sym)
            word_completion[t] = sym
            res_word[t] = '_'
            if res_word.count(sym) > 0:
                while res_word.count(sym) > 0:
                    t = res_word.index(sym)
                    word_completion[t] = sym
                    res_word[t] = '_'
            print(* word_completion)
        elif sym not in res_word:
            print('Такой', sym, 'буквы нет в слове...')
            print(* word_completion)
            tries -= 1
            if tries == 0:
                print(display_hangman(tries))
                ...
                print(display_hangman(tries))
                continue
        if res_word.count('_') == len(res_word):
            print(* word_completion)
            return 'Поздравляем, вы угадали слово! Вы победили!'
print(play(get_word()))